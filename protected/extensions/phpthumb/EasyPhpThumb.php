...
class EasyPhpThumb extends CApplicationComponent {
	...
	public function resizeByGHz($width, $height){
	...
	public function save($filename = null, $format = null){
       ...
       if($format!='GIF')
       {
            if (array_search($format, array('GIF','JPG', 'PNG','GIF'))==null) {
				throw new Exception("Given format not found in allow1ed (GIF, JPG, PNG) - received ".$format);
			}
       }
