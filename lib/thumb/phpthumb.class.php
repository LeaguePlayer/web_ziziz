...
class phpthumb {
	...
	var $phpthumb_version = '1.7.10-201104242100';
...
//		}
	...
	function CleanUpCacheDirectory() {
		$this->DebugMessage('CleanUpCacheDirectory() set to purge ('.number_format($this->config_cache_maxage / 86400, 1).' days; '.number_format($this->config_cache_maxsize / 1048576, 2).'MB; '.number_format($this->config_cache_maxfiles).' files)', __FILE__, __LINE__);
		$DeletedKeys = array();
		...
		if (($this->config_cache_maxage > 0) || ($this->config_cache_maxsize > 0) || ($this->config_cache_maxfiles > 0)) {
			...
			foreach ($AllFilesInCacheDirectory as $fullfilename) {
				if (preg_match('#^phpThumb_cache_#i', $fullfilename) && file_exists($fullfilename)) {
					...
					switch ($command) {
						case 'brit':
							if ($this->ImageMagickSwitchAvailable('modulate')) {
								$commandline .= ' -modulate '.(100 + intval($parameter)).',100,100';
						...
						case 'ds':
							if ($this->ImageMagickSwitchAvailable(array('colorspace', 'modulate'))) {
								if ($parameter == 100) {
									...
									$commandline .= ' -modulate 100,'.(100 - intval($parameter)).',100';
								}
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'clr':
							if ($this->ImageMagickSwitchAvailable(array('fill', 'colorize'))) {
								...
								$commandline .= ' -fill '.escapeshellarg('#'.$color).' -colorize '.min(max(intval($amount), 0), 100);
							}
							break;
						...
						case 'sep':
							if ($this->ImageMagickSwitchAvailable('sepia-tone')) {
								...
								if (!$color) {
									$commandline .= ' -sepia-tone '.min(max(intval($amount), 0), 100).'%';
									$successfullyProcessedFilters[] = $filterkey;
								}
							}
							break;
						...
						case 'gam':
							if ($this->ImageMagickSwitchAvailable('gamma')) {
								$commandline .= ' -gamma '.min(max(floatval($amount), -10), 10);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'th':
							if ($this->ImageMagickSwitchAvailable(array('threshold', 'dither', 'monochrome'))) {
								$commandline .= ' -threshold '.round(min(max(intval($amount), 0), 255) / 2.55).'% -dither -monochrome';
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'edge':
							if ($this->ImageMagickSwitchAvailable('edge')) {
								...
								$commandline .= ' -edge '.(!empty($parameter) ? intval($parameter) : 1);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'emb':
							if ($this->ImageMagickSwitchAvailable(array('emboss', 'negate'))) {
								$parameter = (!empty($parameter) ? $parameter : 2);
								$commandline .= ' -emboss '.intval($parameter);
								if ($parameter < 2) {
							...
							if ($this->ImageMagickSwitchAvailable(array('border', 'bordercolor', 'thumbnail', 'crop'))) {
								if (!$this->zc) {
									...
									if ($width && !$rX && !$rY) {
										...
										$commandline .= ' -border '.intval($width).' -bordercolor '.escapeshellarg('#'.$color);
